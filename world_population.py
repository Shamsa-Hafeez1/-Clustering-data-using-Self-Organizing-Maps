...
def clean_data(path): 
    ...
    df = df.drop(df[df.apply(lambda x: x.astype(str).str.contains('N.A.').any(), axis=1)].index)
    id_col = "Country (or dependency)" #input("Enter the name of ID column: ")
...
no_of_cls = 100
epochs = 2
path = 'E:\Computational Intelligence\Assignment 3\population_by_country_2020.csv'
...
for i in range(len(id)): 
    try: 
        world[world.name == id[i]].plot(color=colors[i],ax=ax)
    except: 
